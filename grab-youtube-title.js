...
var videoLinks = process.argv.slice(2);

if (!videoLinks.length) {
    console.log('Please enter video links');
    return;
}
    ...
    service.videos.list({
        ...
        if (videos.length === 0) {
            ...
            videos.forEach((video) => {
                ...
                console.log('channel-id', snippet.channelId);
...
function getVideoIds() {
    return videoLinks.map(link => youtube_parser(link)).join(',');
}
